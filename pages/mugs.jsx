      ...
      <section className="text-gray-600 body-font">
        <div className="container px-5 py-24 mx-auto">
          ...
          <div className="flex flex-wrap m-4 md:pl-4 justify-center">
            {(outOfStock)&& <p> Sorry, All the Mugs are currently out of stock, new stock coming soon. Stay tuned !</p>}
...
for(let item of products){
    if(item.title in mugs){
        if(!(mugs[item.title].color.includes(item.color)) && item.availableQty > 0){
            ...
            if(!mugs[item.title].aslug){

              mugs[item.title].aslug=item.slug;
            }
        }
...
let SALE=process.env.SALE
  return {
    props:{products:JSON.parse(JSON.stringify(mugs)),  SALE:SALE},
